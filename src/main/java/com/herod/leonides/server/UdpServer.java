package com.herod.leonides.server;
 ...
 * 
 * <p>The supporting {@link Event} and {@link UdpListener}
...
public class UdpServer {
    ...
    private final static int PORT_DEFAULT = 10410;
    ...
    private Collection<UdpListener> listeners = new LinkedList<UdpListener>();    // Event listeners
...
/* ********  R U N N I N G  ******** */
    ...
    public synchronized void start(){
        if( this.currentState == ServerState.STOPPED ){ // Only if we're stopped now
            ...
            Runnable run = new Runnable() {
                ...
                public void run() {
                    ...
                    ioThread = null;          
                    setState( ServerState.STOPPED ); // Clear thread
                }   // end run
            };  // end runnable
            ...
            setState( ServerState.STARTING );        // Update state
    ...
    public synchronized void stop(){
        if( this.currentState == ServerState.STARTED ){         // Only if already STARTED
            setState( ServerState.STOPPING );                // Mark as STOPPING
    ...
    protected synchronized void setState( ServerState state ){
        ServerState oldVal = this.currentState;
            ...
            case STARTED:
                this.addPropertyChangeListener(STATE_PROP, new PropertyChangeListener() {
                    public void propertyChange(PropertyChangeEvent evt) {
                        ServerState newState = (ServerState)evt.getNewValue();
    ...
    protected void runServer(){
        try{
            ...
            setState( ServerState.STARTED );                                   // Mark as started
            LOGGER.info( "UDP Server listening..." );
            
            while( !this.mSocket.isClosed() ){
                synchronized( This ){
                    if( this.currentState == ServerState.STOPPING ){
            ...
            synchronized( This ){
                if( this.currentState == ServerState.STOPPING ){  // User asked to stop
                    this.mSocket.close();
...
/* ********  P A C K E T  ******** */    
     ...
     * @throws IOException if the server throws an exception
...
/* ********  E V E N T S  ******** */
    ...
    /** Adds a {@link UdpListener}.
     * @param l the UdpServer.Listener
     */
    public synchronized void addUdpServerListener(UdpListener l) {
        listeners.add(l);
    }

    /** Removes a {@link UdpListener}.
     * @param l the UdpServer.Listener
     */
    public synchronized void removeUdpServerListener(UdpListener l) {
        listeners.remove(l);
    }
    ...
    protected synchronized void fireUdpServerPacketReceived() {
        ...
        UdpListener[] ll = listeners.toArray(new UdpListener[ listeners.size() ] );
        for( UdpListener l : ll ){
            try{
...
/* ********                                                          ******** */    
     ...
     * @see Event
     */
    public static interface UdpListener extends java.util.EventListener {
         ...
         * @see Event#getPacket
         */
        public abstract void packetReceived(Event evt);
...
/* ********                                                        ******** */    
     ...
     * {@link Listener} interface with empty methods.
     * 
     * <p>This code is released into the Public Domain.
     * Since this is Public Domain, you don't need to worry about
     * licensing, and you can simply copy this UdpServer.java file
...
//    public class Adapter implements UdpServer.Listener {
        ...
        /**
         * Empty call for {@link Listener#udpServerPacketReceived}.
         * @param evt the event
         */
//        @Override
//        public void udpServerPacketReceived( UdpServer.Event evt ) {}
